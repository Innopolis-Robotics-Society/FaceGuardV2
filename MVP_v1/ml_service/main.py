"""Real ML service for FaceGuard.

Replaces ml_stub. Runs on Raspberry Pi or laptop.
Uses InsightFace (buffalo_sc) for detection + embeddings.
Uses MediaPipe FaceMesh for liveness detection (blink detection).
"""
...
def _calculate_ear(landmarks: np.ndarray, eye_idx: list[int]) -> float:
    ...
    Expected landmarks array shape: (N, 2), containing absolute coordinates.
    """
...
def process_frame(frame: np.ndarray) -> tuple[bytes, dict]:
    ...
    # Вызов MediaPipe только при наличии лиц в кадре
    h_img, w_img, _ = frame.shape
    mp_results = mp_face_mesh.process(rgb_frame)
    
    main_face_landmarks = None
    if mp_results.multi_face_landmarks:
        # Извлечение и денормализация координат точек
    ...
    for i, face in enumerate(faces):
        ...
        if i == 0:
            ...
            if ear_avg > 0.0 and ear_avg < EAR_THRESHOLD:
                blink_counter += 1
            elif ear_avg > 0.0:
            ...
            status_text = "LIVE" if is_live else "BLINK TO UNLOCK"
...
app = FastAPI(title="FaceGuard ML Service", version="1.1.0", lifespan=lifespan)
